...
def numpy_tenpi_ccsd(nocc: int, nvir: int, F: list, V: list, T: list, type_: type = np.complex128) -> list:
    ...
    E1 += oe.contract('ijab,ai->jb',V7,T1, optimize='optimal')
    
    #Contraction 3; Tree Level  2; Scaling  1/ 1 Result_size  1/ 1
    E1 += 2.0 * oe.contract('jb->jb',F3)
    
    #Contraction 4; Tree Level  1; Scaling  2/ 2 Result_size  0/ 0
    Z0 += 0.5 * oe.contract('bj,jb->',T1,E1, optimize='optimal')
    ...
    A3 += -2.0 * oe.contract('ck,jkci->ji',T1,E2, optimize='optimal')
    
    del E2
    
    #Contraction 21; Tree Level  2; Scaling  2/ 0 Result_size  2/ 0
    A3 += oe.contract('ji->ji',A2)
    
    #Contraction 22; Tree Level  1; Scaling  3/ 1 Result_size  1/ 1
    Z1 += 0.5 * oe.contract('aj,ji->ai',T1,A3, optimize='optimal')
    ...
    E4 += oe.contract('akcd,dj->akcj',V8,T1, optimize='optimal')
    
    G4 = np.zeros([nvir, nocc, nocc, nocc], dtype=type_)
    
    #Contraction 38; Tree Level  3; Scaling  3/ 5 Result_size  3/ 1
    G4 += oe.contract('akcd,cdij->akij',V8,T2, optimize='optimal')
    ...
    M4 += oe.contract('akcd,ci->akdi',V8,T1, optimize='optimal')
    
    X4 = np.zeros([nvir, nocc, nocc, nocc], dtype=type_)
    
    #Contraction 44; Tree Level  3; Scaling  3/ 3 Result_size  3/ 1
    X4 += oe.contract('dbij,kd->bkij',T2,Y3, optimize='optimal')
    ...
    Y4 += oe.contract('acij,bc->abij',T2,E3, optimize='optimal')
    ...
    Z2 += -0.5 * oe.contract('baij->abij',Y4)
    ...
    I5 += oe.contract('dblj,kldi->bkji',T2,X1, optimize='optimal')
    
    J5 = np.zeros([nocc, nvir, nocc, nocc], dtype=type_)
    
    #Contraction 60; Tree Level  3; Scaling  3/ 3 Result_size  3/ 1
    J5 += oe.contract('kc,cbij->kbij',F3,T2, optimize='optimal')
    ...
    X5 += oe.contract('ci,abcj->abij',T1,M5, optimize='optimal')
    ...
    Z2 += -0.5 * oe.contract('abji->abij',X5)
    ...
    Y7 += oe.contract('klic,ck->li',V4,T1, optimize='optimal')
